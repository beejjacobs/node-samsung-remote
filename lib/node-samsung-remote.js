...
class SamsungRemote {
   ...
   */
  previousChannel() {
    this.log('previousChannel');
    this.send(SamsungRemote.keys.PRECH);
  }
  ...
  tv() {
    this.log('tv');
    this.send(SamsungRemote.keys.TV);
  }
  ...
  hdmi() {
    this.log('hdmi');
    this.send(SamsungRemote.keys.HDMI);
  }
   ...
   */
  processBuffer() {
    this.log('processBuffer commandBuffer.length:' + this.commandBuffer.length + ' processing:' + this.processing);
  ...
  sendToDevice(command) {
    ...
    socket.on('connect', () => {
      let message = this.buildMessage(command);
    ...
    socket.on('close', () => {
      this.log('disconnected from ' + this.ip + ':' + this.port);
    });
